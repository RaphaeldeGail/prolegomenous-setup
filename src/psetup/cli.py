def main():
    ...
    project = project.generate_project(credentials=credentials, parent=setup['parent'], executive_group=setup['google']['groups']['executive_group'])
    print('generating root tag... ', end='')
    root_tag = tag.generate_root_tag(credentials=credentials, parent=setup['parent'])
    if not root_tag.is_bound(credentials=credentials, project=project.name):
        print('binding root tag... ', end='')
        bound = root_tag.bind(credentials=credentials, project=project.name)
    ...
    workspace_tag = tag.generate_workspace_tag(credentials=credentials, parent=setup['parent'], builder_email=service_account.name.split('/serviceAccounts/')[1])
